class MyLinkedList:
    ...
    #返回linkedlist中第index個節點的值。
    #如果index無效（小於0或大於等於linkedlist的長度），則返回-1。
    ...
    #在linkedlist的第一個元素之前添加一個值節點。插入後，新節點將成為linkedlist的第一個節點。
    def addAtHead(self, val: int) -> None:
        self.linkedlist.insert(0,val)

    #將值節點添加到linkedlist的最後一個元素。
    def addAtTail(self, val: int) -> None:
        self.linkedlist.append(val)

    #在linkedlist的第index個節點之前添加一個值節點。
    #如果index等於linkedlist的長度，則添加到末端。
    #如果index大於linkedlist的長度，則不會插入。
    ...
    #如果index有效（大於等於0和小於linkedlist的長度），則刪除linkedlist中第index個節點。
